...
const displayValue = computed(() => selected.value ? `${selected.value.name}` : 'Lied auswählen');
...
<template>
    <div class="relative min-w-0">
        ...
        <button class="flex min-h-10 w-full items-center justify-between rounded-md border border-[#cfd6dc] bg-white px-[11px] py-2 text-left text-[#1f2933] focus:outline focus:outline-2 focus:outline-[#b6c5dc]" type="button" :aria-expanded="isOpen" @click="open">
            <span :class="!selected ? 'text-[#89939d] font-normal' : ''">{{ displayValue }}</span>
        ...
        <div v-if="selected" class="mt-1 flex items-center justify-between gap-2 text-[11px] text-[#66717d]">
            <span><strong>{{ selected.category?.name ?? 'ChurchTools' }}</strong> · {{ selected.name }}</span>
        ...
        <Card v-if="isOpen" class="absolute left-0 right-0 top-[calc(100%+8px)] z-[5] shadow-[0_14px_32px_rgba(25,37,48,.17)]" :data-cy="`song-picker-${label}`">
            ...
            <div v-else class="mt-2.5 grid max-h-60 overflow-auto" role="listbox">
                <button v-for="song in results" :key="song.id" class="grid grid-cols-[92px_minmax(0,1fr)] gap-2 border-0 border-t border-[#edf0f2] bg-transparent px-[5px] py-[9px] text-left text-[#1f2933] hover:bg-[#f3f6f9]" type="button" role="option" @click="choose(song)">
                    <span class="text-[11px] font-bold text-[#4b5d79]">{{ song.category?.name ?? 'Song' }}</span>
                    <span>{{ song.name }}</span>
                    <span v-if="song.author" class="col-start-2 text-[11px] text-[#66717d]">{{ song.author }}</span>
                </button>
            </div>
        </Card>
    </div>
