...
export const generateNormalizedAgenda = (input: AgendaGenerationInput): NormalizedAgenda => {
    ...
    const addText = (nodeId: string, title: string, note?: string) => {
        items.push({ nodeId, position: items.length, type: 'text', title, ...(note ? { note } : {}) });
    };
    ...
    const renderReading = (node: ReadingSlotNode) => {
        ...
        addText(node.id, titleFor(node.label, node.slot === 'psalm' ? 'Psalm' : 'Lesung'), resolved);
    };
    ...
    const renderSermon = (node: SermonSlotNode) => {
        ...
        addText(node.id, titleFor(node.label, 'Predigt'), resolved);
    };
    ...
    const renderFreeText = (node: FreeTextSlotNode) => {
        ...
        addText(node.id, titleFor(node.label, node.slot), value.trim());
    };
    ...
    const addFixedText = (node: FixedTextNode) => addText(node.id, titleFor(node.label, node.text));
    const addRubric = (node: RubricNode) => addText(node.id, titleFor(node.label, node.text), node.label ? node.text : undefined);
    const addCreed = (node: CreedNode) => addText(node.id, titleFor(node.label, node.creed ?? 'Glaubensbekenntnis'));
    const addPrayer = (node: PrayerNode) => addText(node.id, titleFor(node.label, node.text ?? 'Gebet'));
